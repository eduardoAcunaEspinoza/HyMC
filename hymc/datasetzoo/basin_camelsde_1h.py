...
class BasinCamelsDE_1h(Basin):
    """Class to process data from the CAMELS Germany dataset in hourly resolution by [1]_ . 
    
    The class inherits from Basin to create a calibration object following the spotpy library [2]_. However here we 
    code the _read_data method, that specify how we should read the information from CAMELS-DE-1h.
    ...
    warmup_period: int
        Number of timesteps (e.g. hours) to warmup the internal states of the conceptual model
    path_additional_features: Union[str, None]
        Path to a pickle or netCDF file containing additional features that will be used during calibration

    References
    ----------
    .. [1] TODO Add once published
    ...
    def __init__(
        ...
        super(BasinCamelsDE_1h, self).__init__(
            model=model,
            path_data=path_data,
            basin_id=basin_id,
    ...
    def _read_data(self) -> Dict[str, pd.DataFrame]:
        ...
        # load time series
        df = read_data_camelsde_1h(path_data=self.path_data, basin_id=self.basin_id)

        # Add additional features in case there are any
        if self.additional_features:
            df = pd.concat([df, self.additional_features[self.basin_id]], axis=1)
        
        # Filter for specific time period. If custom time periods are used, we run the model for the whole period and 
        # then filter the training/testing subsets.
    ...
    def check_basins(path_data: str,
        ...
        for basin in basins_id:
            ...
            df = read_data_camelsde_1h(path_data=path_data, basin_id=basin)
...
def read_data_camelsde_1h(path_data: str, basin_id: str) -> pd.DataFrame:
    """Read data for a specific basin from the CAMELS-DE-1h dataset
    ...
    """
    path_timeseries = f"{path_data}/timeseries/CAMELS_DE_1h_hydromet_timeseries_{basin_id}.csv"
